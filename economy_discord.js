...
function economyCommandData() {
    return [
        ...
        new Discord.SlashCommandBuilder().setName('blackjack').setDescription('Play public blackjack with administrator-set limits')
            .addIntegerOption(option => option.setName('amount').setDescription('Bet amount (admin-configured limits)').setMinValue(1).setRequired(true)),
        new Discord.SlashCommandBuilder().setName('duel').setDescription('Challenge another member to a 50/50 Blood Money duel')
            ...
            .addIntegerOption(option => option.setName('amount').setDescription('Equal wager from each player').setMinValue(1).setRequired(true)),
        new Discord.SlashCommandBuilder().setName('economy').setDescription('Administer the Blood Money economy')
            .addSubcommand(sub => sub.setName('add').setDescription('Add Blood Money').addUserOption(o => o.setName('user').setDescription('Member').setRequired(true)).addIntegerOption(o => o.setName('amount').setDescription('Amount').setMinValue(1).setRequired(true)))
            .addSubcommand(sub => sub.setName('remove').setDescription('Remove Blood Money').addUserOption(o => o.setName('user').setDescription('Member').setRequired(true)).addIntegerOption(o => o.setName('amount').setDescription('Amount').setMinValue(1).setRequired(true)))
            .addSubcommand(sub => sub.setName('set').setDescription('Set a Blood Money balance').addUserOption(o => o.setName('user').setDescription('Member').setRequired(true)).addIntegerOption(o => o.setName('amount').setDescription('Balance').setMinValue(0).setRequired(true)))
...
function createEconomyIntegration(client, economy, options = {}) {
    ...
    async function handleCommand(interaction) {
        ...
        try {
            ...
            } else if (name === 'economy') await handleAdmin(interaction);
        } catch (error) {
...
module.exports = { economyCommandData, createEconomyIntegration, pokerComponents, canAdministerEconomy };
