...
function membershipsNotEnabled(error) { return error?.youtubeReason === 'channelMembershipsNotEnabled' || /channel does not have Channel Memberships enabled/i.test(String(error?.message || '')); }
...
async function load(){try{const d=await api('/api/memberships');streamerCountEl.textContent=d.streamers.length;memberCountEl.textContent=d.linkCount;const dates=d.streamers.map(x=>x.lastSyncAt).filter(Boolean).sort();lastSyncEl.textContent=dates.length?new Date(dates.at(-1)).toLocaleString():'Never';streamersEl.innerHTML=d.streamers.length?d.streamers.map(s=>card(s,d.roles)).join(''):'<div class="panel empty">Add the first supported streamer above.</div>';auditEl.innerHTML=d.audit.length?d.audit.map(a=>'<div class="audit"><b>'+h(a.event)+'</b> · '+new Date(a.at).toLocaleString()+'<br><span class="muted">'+h(a.details)+'</span></div>').join(''):'<p class="muted">No membership activity yet.</p>'}catch(e){alert(e.message)}}
function card(s,roles){const tierHtml=s.tiers.length?s.tiers.map(t=>'<div class="tier"><b>'+h(t.displayName)+'</b><select data-tier="'+h(t.youtubeLevelId)+'"><option value="">No Discord role</option>'+roles.map(r=>'<option value="'+r.id+'" '+(r.id===t.discordRoleId?'selected':'')+'>'+h(r.name)+'</option>').join('')+'</select><button onclick="mapTier(\\''+s.id+'\\',this)">Save role</button></div>').join(''):'<p class="muted">'+(s.connected?'No membership tiers are available yet. Enable paid Channel Memberships, then select Sync now.':'Tiers appear after the creator connects YouTube.')+'</p>';return '<section class="panel" data-id="'+s.id+'"><div class="head"><div><h2>'+h(s.displayName)+'</h2><div class="'+(s.connected?'status':'bad')+'">'+(s.connected?'Connected: '+h(s.channelTitle||s.channelId):'Waiting for creator connection')+'</div><p class="muted">Grace period: '+s.graceDays+' days · '+(s.enabled?'sync enabled':'sync paused')+(s.lastError?'<br><span class="bad">'+h(s.lastError)+'</span>':'')+'</p></div><div class="buttons"><button onclick="invite(\\''+s.id+'\\',this)">Creator link</button><button class="secondary" onclick="syncOne(\\''+s.id+'\\')">Sync now</button><button class="secondary" onclick="edit(\\''+s.id+'\\','+s.graceDays+','+s.enabled+')">Edit</button><button class="danger" onclick="removeOne(\\''+s.id+'\\')">Delete</button></div></div><div class="creator-link" hidden></div><div class="tiers">'+tierHtml+'</div></section>'}
addButton.onclick=async()=>{try{await api('/api/memberships',{method:'POST',body:JSON.stringify({displayName:nameInput.value,expectedChannelId:channelInput.value,graceDays:graceInput.value})});nameInput.value='';channelInput.value='';await load()}catch(e){alert(e.message)}};
async function invite(id,button){try{const d=await api('/api/memberships/'+id+'/invite',{method:'POST'});const row=button.closest('section').querySelector('.creator-link');row.replaceChildren();const input=document.createElement('input');input.readOnly=true;input.setAttribute('aria-label','Private creator setup link');input.value=d.url;const copy=document.createElement('button');copy.className='secondary';copy.textContent='Copy link';copy.onclick=async()=>{try{await navigator.clipboard.writeText(input.value);copy.textContent='Copied!';setTimeout(()=>copy.textContent='Copy link',1800)}catch{input.select();document.execCommand('copy')}};const open=document.createElement('a');open.className='btn secondary';open.href=d.url;open.target='_blank';open.rel='noopener';open.textContent='Open test link';row.append(input,copy,open);row.hidden=false}catch(e){alert(e.message)}}
    ...
    async handle(req, res, url, isAuthed) {
        ...
        if (url.pathname === '/membership/google/callback' && req.method === 'GET') {
            try {
                ...
                const channel = await creatorChannel(tokens.access_token, this.fetch);
                let tiers = [];
                let tierNotice = '';
                try { tiers = await membershipLevels(tokens.access_token, this.fetch); }
                catch (error) {
                    if (!membershipsNotEnabled(error)) throw error;
                    tierNotice = ' Channel Memberships is not enabled on this channel yet, so no paid tiers were imported. Once it is enabled, return to the dashboard and select Sync now.';
                }
                ...
                if (tierNotice) this.store.audit('creator-connected-without-memberships', state.streamerId, '', `${channel.title} (${channel.id})`);
                send(res, 200, 'text/html; charset=utf-8', messagePage('YouTube connected', `${channel.title} is securely connected.${tierNotice || ' The server owner can now map your membership tiers to Discord roles.'}`, true));
...
module.exports = { MembershipWeb, adminPage, messagePage, discordExchange, membershipsNotEnabled };
