...
A dedicated public gambling channel can be configured for `/dice`, `/higher-lower`, `/dragon-tower`, `/poker`, `/blackjack`, and `/duel`. The progressive games and `/dice` cap each opening wager at 10% of the richest current member balance in the server. Dice uses a fixed weighted table with 100×, 25×, 10×, 5×, 3×, 2×, 1×, and 0× outcomes. Its return to player is exactly 84.5% for a 15.5% house edge, and the 100× jackpot has a 0.02% (1-in-5,000) chance. Higher / Lower uses a 1.5×, 2×, 3×, 5×, 8×, 12×, 18×, and 25× cash-out ladder calibrated to 85% RTP. Dragon Tower is a 4×8 grid: rows 1–5 hide three eggs and one trap, rows 6–8 hide one egg and three traps, and every successfully cleared row can be cashed out. Its row multipliers are calibrated to the same 85% RTP. Inactivity after progress safely cashes out either progressive game; inactivity before the first choice loses the opening wager. Poker is rendered as a public text table: the initial hand, held-card state, automatic timeout draw, final hand, payout, and balance remain visible to everyone while button security limits Hold and Draw to the player. The Blood Money page includes a **Push / repair heist panel** action for immediate manual publication.
