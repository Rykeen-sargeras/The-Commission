...
function editorPage() {
  ...
  let info=null,state=null,dirty=new Map();
  ...
  async function loadGuild(){if(dirty.size&&!confirm('Discard your unpushed permission changes?')){return;}dirty.clear();updateDirty();const guildId=guildEl.value;if(!guildId)return;roleEl.disabled=true;setStatus('Loading roles and channels…');try{info=await api('/api/permissions/guild?guildId='+encodeURIComponent(guildId));roleEl.innerHTML='<option value="">Choose a role</option>'+info.roles.map(r=>'<option value="'+r.id+'">'+esc(r.name)+(r.managed?' [managed]':'')+'</option>').join('');roleEl.disabled=false;headEl.innerHTML='<tr><th>Channel</th>'+info.permissions.map(p=>'<th title="'+esc(p.key)+'">'+esc(p.label)+'</th>').join('')+'</tr>';rowsEl.innerHTML='';setStatus(info.channels.length+' channels · '+info.roles.length+' roles loaded.',true);}catch(e){setStatus(e.message);}}
  async function loadRole(){if(dirty.size&&!confirm('Discard your unpushed permission changes?')){return;}dirty.clear();updateDirty();const roleId=roleEl.value;if(!roleId||!info)return;setStatus('Loading permission state…');try{state=await api('/api/permissions/role-state?guildId='+encodeURIComponent(guildEl.value)+'&roleId='+encodeURIComponent(roleId));renderRows();setStatus('Click any checkbox to change that permission. Dashed boxes are inherited.',true);}catch(e){setStatus(e.message);}}
  function renderRows(){const stateMap=new Map(state.channels.map(c=>[c.id,c.permissions]));const q=searchEl.value.trim().toLowerCase();rowsEl.innerHTML=info.channels.filter(c=>!q||c.name.toLowerCase().includes(q)||c.typeLabel.toLowerCase().includes(q)).map(c=>{const perms=stateMap.get(c.id)||{};return '<tr class="'+(c.typeLabel==='Category'?'category':'')+'"><td><div class="channel"><span class="kind">'+esc(c.typeLabel)+'</span><span class="channel-name">'+esc(c.name)+'</span></div></td>'+info.permissions.map(p=>{const s=perms[p.key]||{allowed:false,explicit:'inherit'};return '<td data-cell="'+c.id+'|'+p.key+'"><input class="check '+(s.explicit==='inherit'?'inherited':'')+'" type="checkbox" '+(s.allowed?'checked':'')+' data-channel="'+c.id+'" data-perm="'+p.key+'" title="'+esc(s.explicit)+'"></td>';}).join('')+'</tr>';}).join('');document.querySelectorAll('.check').forEach(box=>box.onchange=()=>{const key=dirtyKey(box.dataset.channel,box.dataset.perm);dirty.set(key,{channelId:box.dataset.channel,permission:box.dataset.perm,value:box.checked});box.classList.remove('inherited');box.closest('td').classList.add('changed');updateDirty();});}
