...
function creatorGooglePreflightHelp() {
    return `<section class="warning"><h2>Complete these checks before Google sign-in</h2>
    <ol>
    <li>Use the Google account that is the <b>owner or primary owner</b> of the exact YouTube creator channel. Brand Account managers may need the primary owner to connect it.</li>
    <li>In <a href="https://studio.youtube.com/" target="_blank" rel="noopener noreferrer">YouTube Studio</a>, select that channel and confirm <b>Earn &gt; Memberships</b> is active.</li>
    <li>During this testing period, The Commission owner must approve your Google email as an OAuth test user <b>before</b> you continue. Send the owner the exact Gmail/Google email you will select on Google.</li>
    </ol>
    <p><b>If Google shows “Error 403: access_denied”:</b> your email is not on the app's Google test-user list. This happens on Google's site before it can return you to The Commission. Send the displayed email to The Commission owner, who must add it under <b>Google Auth Platform &gt; Audience &gt; Test users</b>. Wait a few minutes, then reopen your creator link and try again.</p>
    <p class="muted">Do not create a new Google Cloud project or paste client credentials into The Commission. The app owner manages the shared OAuth application.</p></section>`;
}
...
class MemberBridgeWeb {
    ...
    async handle(req, res) {
        ...
        if (req.method === 'GET' && url.pathname === '/creator/help') return send(res, 200, page('Creator connection help', `${creatorGooglePreflightHelp()}${creatorApiSetupHelp()}`));
        ...
        if (req.method === 'GET' && creatorLogin) {
            ...
            return send(res, 200, page('Sign in to creator portal', `<p>Continue as <b>${escapeHtml(creator.display_name)}</b> using the Google account that owns the connected YouTube channel.</p>${creatorGooglePreflightHelp()}<a class="button" href="/creator/login/${creator.id}/start">I completed the checks — Continue with Google</a>`));
        }

        const creatorLoginStart = url.pathname.match(/^\/creator\/login\/(\d+)\/start$/);
        if (req.method === 'GET' && creatorLoginStart) {
            const creator = this.store.getCreator(Number(creatorLoginStart[1]));
            if (!creator?.youtube_channel_id) return send(res, 404, page('Creator not connected', '<p>Ask The Commission owner for a fresh creator invitation.</p>'));
